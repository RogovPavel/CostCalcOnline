...
class TariffsController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', 'actions'=>array('index'), 'roles'=>array('view_tariffs'),),
            array('allow', 'actions'=>array('create'), 'roles'=>array('create_tariffs'),),
            array('allow', 'actions'=>array('update'), 'roles'=>array('update_tariffs'),),
            array('allow', 'actions'=>array('delete'), 'roles'=>array('delete_tariffs'),),
            array('deny',
                    'users'=>array('*'),
            ),
        );
    ...
    public function actionCreate() {
        ...
        try {
            ...
            if (isset($_POST['tariffs'])) {
                if ($model->validate()) {
                    $model->setAttributes($_POST['tariffs']);
                    $model->user_create = Yii::app()->user->user_id;
                    $res = $model->insert();
                    $result['out'] = $res;
                    $result['id'] = $res['data']['tariff_id'];
                }
                return;
            }
    ...
    public function actionUpdate() {
        ...
        try {
            ...
            if (isset($_POST['tariff_id']))
                $model->get_by_id($_POST['tariff_id']);
